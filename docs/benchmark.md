...
## 基准结果（v0.6.0，Z-slab 流式转换 + lz4 压缩）

### 20GB（801×2405×2501，lz4 压缩）

存储比 0.443x → 存储得分 20/20

| 测试项 | v0.5.1 最优 | **v0.6.0** | 变化 |
|--------|-----------|-----------|------|
| X random (100片) | 15.05s | **74.14s** | — |
| Y random (100片) | 47.00s | **52.24s** | +11.1% |
| Z random (100片) | 41.42s | **47.66s** | +15.1% |
| X continuous (10片) | 1.59s | **5.01s** | — |
| Y continuous (10片) | 1.41s | **1.71s** | +21.3% |
| Z continuous (10片) | 1.32s | **1.63s** | +23.5% |
| **T_composite** | **17.49s** | **30.40s** | +73.8% |
| 存储比 | 0.932x | 0.443x | -52.5% |

注：v0.5.1 使用 sidecar stride=1 加速 X 切片（0.932x），v0.6.0 无 sidecar（0.443x）。X random 从 15s 升至 74s 因无 sidecar 加速，但存储比大幅改善。v0.6.0 的 sidecar 留待后续集成。

### 20GB（801×2405×2501，无压缩）

存储比 1.075x → 存储得分 20/20

| 测试项 | v0.6.0 冷缓存 | **v0.6.0 热缓存** |
|--------|:---:|:---:|
| X random (100片) | 208.49s | **100.90s** |
| Y random (100片) | 77.25s | **76.74s** |
| Z random (100片) | 69.30s | **81.56s** |
| X continuous (10片) | 7.50s | **9.12s** |
| Y continuous (10片) | 2.34s | **2.88s** |
| Z continuous (10片) | 2.18s | **2.73s** |
| **T_composite** | **61.18s** | **45.65s** |
| 存储比 | 1.075x | 1.075x |

### 50GB（2001×2201×3000，lz4 压缩）

存储比 0.996x → 存储得分 20/20

| 测试项 | v0.5.1 最优 | **v0.6.0** | 变化 |
|--------|-----------|-----------|------|
| X random (100片) | 194.86s | **216.99s** | +11.4% |
| Y random (100片) | 178.17s | **230.21s** | +29.2% |
| Z random (100片) | 164.90s | **223.66s** | +35.6% |
| X continuous (10片) | 6.84s | **8.94s** | +30.7% |
| Y continuous (10片) | 6.17s | **8.25s** | +33.7% |
| Z continuous (10片) | 4.35s | **6.08s** | +39.8% |
| **T_composite** | **104.74s** | **115.69s** | +10.5% |
| 存储比 | 0.996x | 0.996x | — |

### 50GB（2001×2201×3000，无压缩）

存储比 1.044x → 存储得分 20/20

| 测试项 | v0.6.0 冷缓存 | **v0.6.0 热缓存** |
|--------|:---:|:---:|
| X random (100片) | 488.67s | **246.03s** |
| Y random (100片) | 252.07s | **251.26s** |
| Z random (100片) | 202.08s | **191.95s** |
| X continuous (10片) | 9.32s | **8.09s** |
| Y continuous (10片) | 6.43s | **7.16s** |
| Z continuous (10片) | 5.89s | **5.77s** |
| **T_composite** | **160.74s** | **118.38s** |
| 存储比 | 1.044x | 1.044x |

### v0.5.1 vs v0.6.0 总结

| 配置 | v0.5.1 T_composite | v0.6.0 T_composite | 变化 |
|------|-------------------|-------------------|------|
| 20GB lz4+sidecar | 17.49s (0.932x) | — | — |
| 20GB lz4 无 sidecar | — | 30.40s (0.443x) | — |
| 50GB lz4 | 104.74s (0.996x) | 115.69s (0.996x) | +10.5% |

v0.6.0 是纯 Z-slab 流式转换的实现，sidecar 支持留待后续版本集成。X 切片在无 sidecar 情况下受限于跨 superblock 跳跃读取。
...
| **v0.6.0** | **Z-slab 流式转换 + per-block lz4** | **30.40s** | **115.69s** |
...
- **Z-slab 流式转换**：按 Z 方向 64 层为一批，每批一次大块顺序 pread，最大化 HDD 带宽利用率
- **per-block lz4 压缩**：每个 superblock 独立决策压缩存储，20GB 数据 0.443x 压缩率
- **verify mmap 优化**：采样模式改用 mmap 零拷贝访问，消除逐点 pread 的 HDD 随机寻道瓶颈
- **转换效率**：20GB 381s（无压缩）/ 161s（lz4），50GB 962s（无压缩）/ 950s（lz4），峰值内存均 ~1098 MB
