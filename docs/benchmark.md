...
注：v0.5.1 使用 sidecar stride=1 加速 X 切片（0.932x），v0.6.0 无 sidecar（0.443x），存储比更优但 X 切片性能退步。Sidecar 留待后续集成。
...
注：v0.6.0 是纯 Z-slab 流式转换的实现，sidecar 支持留待后续版本集成。X 切片在无 sidecar 情况下受限于跨 superblock 跳跃读取。
