...
## 基准结果（v0.6.0，Z-slab 流式转换 + 压缩窗口合并，当前最优）

### 20GB（801×2405×2501，v0.5.1 ZYX 文件 + v0.6.0 reader）

存储比 0.932x → 存储得分 20/20

| 测试项 | v0.5.1 最优 | **v0.6.0** | 提升 |
|--------|-----------|-----------|------|
| X random (100片) | 15.05s | **9.07s** | **-39.7%** |
| Y random (100片) | 47.00s | **35.95s** | **-23.5%** |
| Z random (100片) | 41.42s | **40.99s** | -1.0% |
| X continuous (10片) | 1.59s | **0.69s** | **-56.6%** |
| Y continuous (10片) | 1.41s | **1.06s** | -24.8% |
| Z continuous (10片) | 1.32s | **1.06s** | -19.7% |
| **T_composite** | **17.49s** | **14.80s** | **-15.4%** |
| 存储比 | 0.932x | 0.932x | — |

注：使用 v0.5.1 生成的 ZYX 压缩文件，v0.6.0 reader 窗口合并压缩读取模式。全轴超越 v0.5.1。Cold cache 也达到 15.61s（-10.7%）。

### 50GB 待解决

v0.6.0 reader + v0.5.1 ZYX 50GB lz4 文件：cold 153.72s（vs v0.5.1 104.74s，+46.8%）。根因：压缩率 0.996x，窗口合并开销 > 收益。计划切 raw 或加近零压缩阈值自动回退 uncompressed 路径。

---

## 基准结果（v0.5.1，存储比 ≤1.5x，当前最优）
...
| **v0.6.0** | **Z-slab 转换 + 压缩窗口合并 + sidecar** | **14.80s** | — |
