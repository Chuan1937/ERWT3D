...
## 基准结果（v0.6.0，Z-slab 流式转换 + v05-yzx 物理布局）
...
|--------|-----------|-----------|------|
| X random (100片) | 15.05s | **31.11s** | — |
| Y random (100片) | 47.00s | **46.96s** | ≈ |
| Z random (100片) | 41.42s | **40.24s** | ≈ |
| X continuous (10片) | 1.59s | **1.77s** | +11.3% |
| Y continuous (10片) | 1.41s | **1.77s** | +25.5% |
| Z continuous (10片) | 1.32s | **1.64s** | +24.2% |
| **T_composite** | **17.49s** | **20.58s** | +17.7% |
| 存储比 | 0.932x | 0.932x | — |

注：v0.5.1 使用 ZYX 布局。v0.6.0 使用 v05-yzx 布局，Y 轴跨步从 1120MB 降至 32MB，Y_random 持平。X_random 退步因 sidecar 集成细节未完全优化，仍有挖掘空间。

### 50GB（2001×2201×3000，lz4 压缩，无 sidecar）

存储比 0.996x → 存储得分 20/20

| 测试项 | v0.5.1 最优 | **v0.6.0** | 变化 |
|--------|-----------|-----------|------|
| X random (100片) | 194.86s | **261.40s** | +34.1% |
| Y random (100片) | 178.17s | **232.11s** | +30.3% |
| Z random (100片) | 164.90s | **201.15s** | +22.0% |
| X continuous (10片) | 6.84s | **9.53s** | +39.3% |
| Y continuous (10片) | 6.17s | **8.75s** | +41.8% |
| Z continuous (10片) | 4.35s | **6.23s** | +43.2% |
| **T_composite** | **104.74s** | **119.86s** | +14.4% |
| 存储比 | 0.996x | 0.996x | — |

注：v0.5.1 使用 LeafOp 紧凑化 + 深度优化的 reader。v0.6.0 是重写 Z-slab 转换引擎，reader 层尚未接回 LeafOp 等优化，转换效率大幅提升但运行时略慢。
...
| **v0.6.0** | **Z-slab 流式转换 + v05-yzx 布局 + sidecar** | **20.58s** | **119.86s** |
