...
|--------|------------|--------|------|
| 20GB | 15.39s | 0.835x | S=64, lz4压缩 + sidecar stride=1, best run |
...
- **X-plane sidecar 是 X random 的最大收益点**：stride=1 全覆盖时 X random 从 63s 降到 14s（-77%），因为只需读压缩 chunk（~9MB/plane）而非扫描整个文件
- **sidecar 压缩率取决于 YZ 平面空间相关性**：20GB 数据集 0.392x（可行），50GB 仅 0.942x（超存储限制自动跳过）
