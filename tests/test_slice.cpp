...
// Compute deterministic value: v(x,y,z) = x + 1000*y + 1000000*z
...
static void testRoundtrip(uint64_t nx, uint64_t ny, uint64_t nz, const char* label) {
    ...
    assert(writeERWT3D(path, orig.data(), nx, ny, nz));
    ...
    for (uint64_t i = 0; i < orig.size(); ++i) {
        assert(std::abs(orig[i] - restored[i]) < 1e-6f);
    }
...
static void testSlice(uint64_t nx, uint64_t ny, uint64_t nz, SliceAxis axis, 
    ...
    assert(reader.readSlice(axis, idx, slice.data()));
    ...
    switch (axis) {
        case SliceAxis::X:
            for (uint64_t z = 0; z < nz; ++z)
                for (uint64_t y = 0; y < ny; ++y)
                    assert(std::abs(slice[z * ny + y] - refVal(idx, y, z)) < 1e-6f);
            break;
        case SliceAxis::Y:
            for (uint64_t z = 0; z < nz; ++z)
                for (uint64_t x = 0; x < nx; ++x)
                    assert(std::abs(slice[z * nx + x] - refVal(x, idx, z)) < 1e-6f);
            break;
        case SliceAxis::Z:
            for (uint64_t y = 0; y < ny; ++y)
                for (uint64_t x = 0; x < nx; ++x)
                    assert(std::abs(slice[y * nx + x] - refVal(x, y, idx)) < 1e-6f);
            break;
    }
...
static void testLineX(uint64_t nx, uint64_t ny, uint64_t nz, uint64_t y, uint64_t z, const char* label) {
    ...
    assert(reader.readLineX(y, z, line.data()));
    
    for (uint64_t x = 0; x < nx; ++x) {
        assert(std::abs(line[x] - refVal(x, y, z)) < 1e-6f);
    }
...
static void testAllSlicesAndLines(uint64_t nx, uint64_t ny, uint64_t nz, const char* label) {
    ...
    // Test slices at boundaries and middle
