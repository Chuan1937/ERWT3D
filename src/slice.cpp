...
// Compute 2D dst index following the official Z-fastest raw layout:
// raw offset(x,y,z) = (x*ny + y)*nz + z.
// X slice (fixed x): output[y * nz + z]  -> out_dim0=nz, out_dim1=ny
// Y slice (fixed y): output[x * nz + z]  -> out_dim0=nz, out_dim1=nx
// Z slice (fixed z): output[x * ny + y]  -> out_dim0=ny, out_dim1=nx
                ...
                for (uint64_t syi = 0; syi < getSuperGridY(header); ++syi) {
                    ...
                    for (uint64_t lz2 = 0; lz2 < leafsPerSuperZ; ++lz2) {
                        for (uint64_t ly2 = 0; ly2 < leafsPerSuperY; ++ly2) {
                            ...
                            instr.base_dst_idx = dstY * nz + dstZ;
                            instr.size_x = 1;
                ...
                for (uint64_t sxi = 0; sxi < getSuperGridX(header); ++sxi) {
                    ...
                    for (uint64_t lz2 = 0; lz2 < leafsPerSuperZ; ++lz2) {
                        for (uint64_t lx2 = 0; lx2 < leafsPerSuperX; ++lx2) {
                            ...
                            instr.base_dst_idx = dstX * nz + dstZ;
                ...
                for (uint64_t sxi = 0; sxi < getSuperGridX(header); ++sxi) {
                    ...
                    for (uint64_t ly2 = 0; ly2 < leafsPerSuperY; ++ly2) {
                        for (uint64_t lx2 = 0; lx2 < leafsPerSuperX; ++lx2) {
                            ...
                            instr.base_dst_idx = dstX * ny + dstY;
                            instr.size_x = std::min(lx, nx - dstX);
                            instr.size_y = std::min(ly, ny - dstY);
                            instr.size_z = 1;
                            instr.src_off_x = 0;
        ...
        for (uint64_t dz = 0; dz < copy.size_z; ++dz) {
            for (uint64_t dy = 0; dy < copy.size_y; ++dy) {
                for (uint64_t dx = 0; dx < copy.size_x; ++dx) {
                    ...
                    switch (plan.axis) {
                        case SliceAxis::X:
                            // output layout: y * nz + z
                            dstIdx = ((copy.base_dst_idx / nz) + dy) * nz + (copy.base_dst_idx % nz + dz);
                            break;
                        case SliceAxis::Y:
                            // output layout: x * nz + z
                            dstIdx = ((copy.base_dst_idx / nz) + dx) * nz + (copy.base_dst_idx % nz + dz);
                            break;
                        case SliceAxis::Z:
                            // output layout: x * ny + y
                            dstIdx = ((copy.base_dst_idx / ny) + dx) * ny + (copy.base_dst_idx % ny + dy);
                            break;
                    }
            ...
            for (uint64_t dy = 0; dy < copy.size_y; ++dy) {
                for (uint64_t dx = 0; dx < copy.size_x; ++dx) {
                    ...
                    switch (plan.axis) {
                        case SliceAxis::X:
                            // output layout: y * nz + z
                            d = ((copy.base_dst_idx / header.nz) + dy) * header.nz + (copy.base_dst_idx % header.nz + dz);
                            break;
                        case SliceAxis::Y:
                            // output layout: x * nz + z
                            d = ((copy.base_dst_idx / header.nz) + dx) * header.nz + (copy.base_dst_idx % header.nz + dz);
                            break;
                        case SliceAxis::Z:
                            // output layout: x * ny + y
                            d = ((copy.base_dst_idx / header.ny) + dx) * header.ny + (copy.base_dst_idx % header.ny + dy);
                            break;
                    }
